...
/**
  ...
  * 参    数：无
  * 返 回 值：无
  * 说    明：硬件I2C的GPIO由SYSCFG_DL_GPIO_init()配置，此处仅做延时
  */
...
void OLED_ShowChar(uint8_t X, uint8_t Y, char Char, uint8_t FontSize)
{
	...
	else if(FontSize == OLED_7X12)	//字体为宽6像素，高8像素
	{
		/*将ASCII字模库OLED_F6x8的指定数据以6*8的图像格式显示*/
...
/**
  * 函    数：在指定范围内显示一个字符
  * 参    数：RangeX 指定字符可以显示范围的左上角的横坐标，范围：负值~127
  * 参    数：RangeY 指定字符可以显示范围的左上角的纵坐标，范围：负值~63
  * 参    数：RangeWidth 指定范围宽度
  * 参    数：RangeHeight 指定范围高度
  * 参    数：X 指定字符左上角的横坐标，范围：负值~127
  * 参    数：Y 指定字符左上角的纵坐标，范围：负值~63
  * 参    数：Char 指定要显示的字符，范围：ASCII码可见字符
  * 参    数：FontSize 指定字体大小
  *           范围：OLED_8X16		宽8像素，高16像素
  *                 OLED_6X8		宽6像素，高8像素
...
/**
  * 函    数：OLED显示混合字符串（汉字与ASCII） BY BILIBILI 上nm网课呢(utf8)
  * 参    数：X 指定汉字串左上角的横坐标，范围：负值~127
  * 参    数：Y 指定汉字串左上角的纵坐标，范围：负值~63
  * 参    数：String 指定要显示的混合字符串，范围：全角字符与半角字符都可以
  *           显示的汉字需要在OLED_Data.c里的OLED_CF16x16或OLED_CF12x12数组定义
  *           未找到指定汉字时，会显示默认图形（一个方框，内部一个问号）
  * 参    数：ChineseFontSize 指定中文文字大小，OLED_12X12或OLED_16X16
  * 参    数：ASCIIFontSize  指定ASCII文字大小,OLED_6X8或OLED_7X12或OLED_8X16
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
void OLED_ShowMixString(uint8_t X, uint8_t Y, char *String, uint8_t ChineseFontSize, uint8_t ASCIIFontSize) {
    ...
    while (*ptr != '\0') {
        if (*ptr < 128) {
            ...
            // 处理UTF-8中文字符(通常为三字节)
            if ((*ptr & 0xF0) == 0xE0) {  // 检查UTF-8三字节编码的头字节
                if (strlen((char*)ptr) >= 3) {  // 确保有足够的字节表示完整的中文字符
                    char Chinese[4]; // 为中文字符分配空间，加上空终止符
                    Chinese[0] = *ptr;
                    Chinese[1] = *(ptr + 1);
                    Chinese[2] = *(ptr + 2);
                    Chinese[3] = '\0'; // 添加终止符
                    OLED_ShowChinese(X, Y, Chinese, ChineseFontSize);
                    X += ChineseFontSize; // 中文字符宽度通常是ASCII宽度的两倍
                    ptr += 3; // 移动指针跳过三字节的中文字符
                } else {
                    break; // 字符串结束或字节不足
                }
            } else {
                // 其他多字节情况（如UTF-8的四字节字符等）可根据需要添加处理代码
                ptr++; // 暂时跳过处理
            }
        }
    }
...
/**
  * 函    数：在指定区域范围内OLED显示混合字符串（汉字与ASCII） BY BILIBILI 上nm网课呢
  * 参    数：RangeX 指定字符可以显示范围的左上角的横坐标，范围：负值~127
  * 参    数：RangeY 指定字符可以显示范围的左上角的纵坐标，范围：负值~63
  * 参    数：RangeWidth 指定范围宽度
  * 参    数：RangeHeight 指定范围高度
...
/**
  ...
  * 参    数：X 指定字符左上角的横坐标，范围：负值~127
  * 参    数：Y 指定字符左上角的纵坐标，范围：负值~63
  * 参    数：String 指定要显示的字符串，范围：ASCII码可见字符
  * 参    数：FontSize 指定字体大小
  *           范围：OLED_8X16		宽8像素，高16像素
  *                 OLED_6X8		宽6像素，高8像素
  *                 OLED_7X12       宽7像素，高12像素
...
/**
  ...
  *           未找到指定汉字时，会显示默认图形（一个方框，内部一个问号）
  * 参    数：FontSize 指定中文文字大小，OLED_12X12或OLED_16X16
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_ShowChinese(uint8_t X, uint8_t Y, char *Chinese, uint8_t FontSize)
{
    uint8_t pChinese = 0;
    uint8_t pIndex;
    uint8_t i;
    ...
    for (i = 0; Chinese[i] != '\0'; i ++)    // 遍历汉字串
    {
        ...
        {
            pChinese = 0;    // 计次归零
            const void* fontArray = FontSize == 12 ? (const void*) OLED_CF12x12 : (const void*) OLED_CF16x16;  
		if(FontSize==12){
            ...
            OLED_ShowImage(X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 12, Y, 12, 12, ((const ChineseCell12x12_t*)fontArray)[pIndex].Data);
		}else{
		...
		OLED_ShowImage(X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, Y, 16, 16, ((const ChineseCell16x16_t*)fontArray)[pIndex].Data);
		}
        }
    }
}
/**
  * 函    数：在指定区域范围内OLED显示汉字串 BY BILIBILI 上nm网课呢
  * 参    数：RangeX 指定字符可以显示范围的左上角的横坐标，范围：负值~127
  * 参    数：RangeY 指定字符可以显示范围的左上角的纵坐标，范围：负值~63
  * 参    数：RangeWidth 指定范围宽度
  * 参    数：RangeHeight 指定范围高度
...
void OLED_ShowChineseArea(uint8_t RangeX,uint8_t RangeY,uint8_t RangeWidth,uint8_t RangeHeight, int16_t X, int16_t Y, char *Chinese, uint8_t FontSize)
{
    ...
    for (i = 0; Chinese[i] != '\0'; i ++)    // 遍历汉字串
    {
		...
		if(FontSize==12){
			...
			OLED_ShowImageArea(X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 12, Y, 12, 12, RangeX, RangeY, RangeWidth, RangeHeight, ((const ChineseCell12x12_t*)fontArray)[pIndex].Data);
			...
			for (pIndex = 0; strcmp(((const ChineseCell16x16_t*)fontArray)[pIndex].Index, "") != 0; pIndex ++)
            {
				...
				OLED_ShowImageArea(X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, Y, 16, 16, RangeX, RangeY, RangeWidth, RangeHeight,((const ChineseCell16x16_t*)fontArray)[pIndex].Data);
...
/**
  ...
  * 参    数：X 指定图像左上角的横坐标，范围：负值~127
  * 参    数：Y 指定图像左上角的纵坐标，范围：负值~63
...
void OLED_ShowImage(int16_t X, int16_t Y, uint16_t Width, uint16_t Height, const uint8_t *Image)
{
    ...
    uint8_t endX = (X + Width - 1 > 127) ? 127 : X + Width - 1; // 计算实际结束显示位置的 X 坐标
    uint8_t endY = (Y + Height - 1 > 63) ? 63 : Y + Height - 1; // 计算实际结束显示位置的 Y 坐标
...
/**
  * 函    数：在指定区域内显示图片 BY BILIBILI上nm网课呢 
  * 参    数：X1，Y1，PictureWidth，PictureHeight图片显示位置,坐标可以为负值
  * 参    数：X2，Y2，AreaWidth，AreaHeight限制显示的区域，坐标可以为负值
  * 参    数：Image图片取模数组
  * 返 回 值：无
  * 说    明：需要调用刷新才可以显示在屏幕上
  */
...
/**
  ...
  * 参    数：X 指定格式化字符串左上角的横坐标，范围：0~127
  * 参    数：Y 指定格式化字符串左上角的纵坐标，范围：0~63
  * 参    数：ChineseFontSize 指定汉字字体大小
  * 参    数：ASCIIFontSize 指定ASCII字体大小
  *           范围：OLED_8X16		宽8像素，高16像素
  *                 OLED_6X8		宽6像素，高8像素
  * 参    数：format 指定要显示的格式化字符串，范围：ASCII码可见字符组成的字符串
  * 参    数：... 格式化字符串参数列表
